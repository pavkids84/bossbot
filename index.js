...
function setBT(msg, date, hour, min, sec)
{
	...
	if (retdate.getTime() < new Date().setHours(9).getTime())
	{
		retdate = null
	}
	return retdate
...
/*
추가 내용
- 알림 생성 시 30분 이후 자동삭제되는 기능
- 점검시간이 넘어가는 시간대에 젠되는 보스는 리스트에서 초기화
- 초기 젠타임을 판단하는 변수를 데이터베이스화
- 보스-알림 채널에 리스트를 한번 출력해 둔 뒤 1분 단위로 수정되는 기능
- 웹페이지 내용을 읽어와 아이모 공지사항을 자동으로 채널에 알리는 기능
- 봇 호스팅 연결하기 (데이터베이스 다 빼버리기)
- 최초젠이면 최초젠이라고 알림
- 보스 참여 카운트 기능
*/
